...
const PointCloudVisualization = () => {
  ...
  const renderPointCloud = (canvas: HTMLCanvasElement, points: any[], rotation: number) => {
    ...
    }).filter(point => point.screenX >= 0 && point.screenX < width && point.screenY >= 0 && point.screenY < height).sort((a, b) => b.z - a.z);
  ...
  return <div className="h-full grid grid-cols-2 grid-rows-2 gap-4">
      {['cam0', 'cam1', 'cam2', 'cam3'].map((cam, index) => <Card key={cam} className="bg-black/20 border-white/20">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-white">Point Cloud {index}</CardTitle>
          </CardHeader>
          <CardContent className="h-[calc(100%-60px)] p-2 py-[20px]">
            <div className="w-full h-full bg-black/40 rounded border border-white/10 overflow-hidden">
              <canvas ref={el => canvasRefs.current[index] = el} width={300} height={200} className="w-full h-full cursor-move" onMouseMove={e => handleMouseMove(index, e)} style={{
