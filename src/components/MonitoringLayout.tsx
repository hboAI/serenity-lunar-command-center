...
const MonitoringLayout = ({ children }: MonitoringLayoutProps) => {
  ...
  const availableTopics = [
    { name: '/motor_status', type: 'plot', description: 'Motor positions, currents, velocities' },
    { name: '/lux', type: 'plot', description: 'Environmental sensors' },
    { name: '/sen66_data', type: 'plot', description: 'Air quality sensors' },
    { name: '/camera/cam*/ir', type: 'image', description: 'IR camera feeds (4 cameras)' },
    { name: '/camera/cam*/pointcloud', type: 'pointcloud', description: 'Point cloud data (4 cameras)' }
  ];
          ...
          <div className="h-full p-4">
            <Card className="h-full bg-black/30 backdrop-blur-xl border border-white/20">
              <CardHeader className="pb-4">
                <CardTitle className="text-2xl text-white">Monitoring Center</CardTitle>
                <TopicSelector 
                  ...
                  <TabsContent value="plots" className="h-[calc(100%-60px)] mt-4">
                    <PlotVisualization 
                      ...
                      selectedTopics={selectedTopics.filter(topic => 
                        plotTopics.some(pt => pt.name === topic)
                      )}
                    />
                  </TabsContent>
